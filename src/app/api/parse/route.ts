...
export const POST = async (request: NextRequest) => {
  ...
  try {
    ...
    logger.debug(
      "Request body parsed",
      { hasGoogleDocsUrl: !!googleDocsUrl, hasJsonData: !!jsonData },
      { requestId },
    );
    ...
    if (googleDocsUrl) {
      try {
        docId = ImprovedParser.getDocumentId(googleDocsUrl);
        logger.parseStart(docId, requestId);
        logger.debug(
          "Document ID extracted",
          { docId, url: googleDocsUrl },
          { requestId },
        );
      } catch {
        ...
        logger.parseError(
          "unknown",
          new Error("Invalid Google Docs URL format"),
          duration,
          requestId,
        );
        return NextResponse.json(
      ...
      logger.debug(
        "Generated temporary doc ID for JSON data",
        { docId },
        { requestId },
      );
    }
    ...
    try {
      ...
      const artist = await ImprovedParser.parseGoogleDoc(googleDocsUrl || "");
      const parseDuration = Date.now() - parseStartTime;

      const stats = {
        artistName: artist.name,
        albumCount: artist.albums.length,
        totalTracks: artist.albums.reduce(
      ...
      if (isQuotaError) {
        ...
        return NextResponse.json(
          {
            error:
              "Google Sheets quota exceeded. Please try again in a few minutes or contact the tracker owner to reduce API usage.",
          },
          {
            status: 429,
            headers: { "X-Request-ID": requestId },
      ...
      logger.apiResponse("POST", "/api/parse", 500, parseDuration, requestId, {
        error: errorMessage,
      });
      return NextResponse.json(
        {
    ...
    logger.apiResponse("POST", "/api/parse", 500, duration, requestId, {
...
export const GET = async (request: NextRequest) => {
  ...
  try {
    ...
    logger.debug(
      ...
      { hasGoogleDocsUrl: !!googleDocsUrl, hasDocId: !!docId },
      { requestId },
    );
    ...
    if (googleDocsUrl) {
      ...
      logger.debug(
        "Reconstructed URL from docId",
        { docId, reconstructedUrl: fullGoogleDocsUrl },
        { requestId },
      );
    } else {
    ...
    try {
      logger.info(
        "Starting document parsing",
        { url: fullGoogleDocsUrl },
        { requestId },
      );

      const parseStartTime = Date.now();
      const artist = await ImprovedParser.parseGoogleDoc(fullGoogleDocsUrl);
      const parseDuration = Date.now() - parseStartTime;

      const documentId = ImprovedParser.getDocumentId(fullGoogleDocsUrl);
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Failed to parse Google Doc";
      logger.apiResponse("GET", "/api/parse", 500, duration, requestId, {
        error: errorMessage,
      });
      return NextResponse.json(
        { error: errorMessage },
