...
export default function ModernMusicPlayer({ track, isVisible, onClose }: ModernMusicPlayerProps) {
  ...
  const getPlayableSource = (track: Track): { type: string, url: string, id?: string, isVideo?: boolean } | null => {
  ...
  const canUseReactPlayer = playable && ['youtube', 'soundcloud', 'vimeo', 'twitch', 'facebook', 'dailymotion', 'video'].includes(playable.type);
  const canUseAudioPlayer = playable && ['audio', 'pillowcase', 'froste'].includes(playable.type);
  ...
  const renderPlayer = () => {
    if (canUseReactPlayer) {
      return (
        <div className="space-y-4">
          {/* Video/Audio Player */}
          <div className={`relative ${isVideo && !audioOnlyMode ? 'aspect-video' : 'h-16'} bg-black rounded-lg overflow-hidden`}>
            ...
            <div className="flex items-center justify-center h-full bg-gray-200 dark:bg-gray-700 rounded">
              <p className="text-gray-600 dark:text-gray-400">Video/Audio player disabled for build</p>
            </div>
          ...
          {isVideo && (
            <div className="flex items-center justify-between">
              ...
              <div className="text-sm text-gray-500 dark:text-gray-400">
                {playable.type.charAt(0).toUpperCase() + playable.type.slice(1)} Player
              </div>
            </div>
          )}
        </div>
    ...
    return (
      <div className="text-center py-8">
        ...
        <button
          onClick={() => window.open(playable.url, '_blank', 'noopener,noreferrer')}
