...
export default function VirtualizedTrackList({ 
  ...
  const {
    ...
    containerHeight: disableVirtualization ? tracks.length * 80 : containerHeight,
    overscan: 5
  });
  ...
  useEffect(() => {
    if (!disableVirtualization && containerRef.current) {
      ...
      const availableHeight = Math.min(maxHeight, window.innerHeight - rect.top - 100);
      ...
      {isLargeDataset && (
        <div className="px-4 py-2 bg-gray-50 dark:bg-gray-700 border-t border-gray-200 dark:border-gray-600">
          <div className="flex items-center justify-center text-xs text-gray-500 dark:text-gray-400">
            ...
            Virtual scrolling enabled for {tracks.length.toLocaleString()} tracks
          </div>
        </div>
      )}
    </div>
